/*
v1.0.1 jQuery baigo ADS 飘动 插件
(c) 2013 baigo studio - http://www.baigo.net/ads/
License: http://www.opensource.org/licenses/mit-license.php
*/

;(function($) {
    $.fn.adsFly = function(options) {
        'use strict';
        ...
        var adPosition = function() {
            winWidth     = $(window).width() - thisObj.find(opts.selector.box).width();
            winHeight    = $(window).height() - thisObj.find(opts.selector.box).height();

            if (xD === 0) {
                ownLeft += i;
                thisObj.find(opts.selector.box).css({
                    left: ownLeft
                });
                if (ownLeft >= winWidth) {
                    ownLeft = winWidth;
        ...
        $.ajax({
            ...
            success: function(_result){ //读取返回结果
                if (typeof _result.posiRow != 'undefined' && typeof _result.advertRows[0] != 'undefined') {
                    ...
                    var imgHover = setInterval(adPosition, _posiOpts.speed);

                    thisObj.find(opts.selector.box).hover(
                        function(){
                            clearInterval(imgHover);
                        },
                        function(){
                            imgHover = setInterval(adPosition, _posiOpts.speed);
                        }
                    );
