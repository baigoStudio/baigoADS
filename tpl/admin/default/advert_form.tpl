					...
					<div class="panel-body">
						<div class="form-group">
							<div id="group_advert_name">
								...
								<input type="text" name="advert_name" id="advert_name" value="{$tplData.advertRow.advert_name}" class="validate form-control">
							</div>
						</div>
						...
						<div class="form-group">
							<div id="group_advert_url">
								...
								<input type="text" name="advert_url" id="advert_url" value="{$tplData.advertRow.advert_url}" class="validate form-control">
							</div>
						</div>

						<div class="form-group">
							<label class="control-label">{$lang.label.posi}<span id="msg_advert_posi_id">*</span></label>
							<select name="advert_posi_id" id="advert_posi_id" class="validate form-control">
							...
							<div class="form-group">
								...
								<div class="panel panel-default panel_dashed">
									<div class="panel-body">
										...
										<div class="form-group">
											<input type="text" name="advert_media_id" id="advert_media_id" value="{$tplData.advertRow.advert_media_id}" class="validate form-control">
						...
						<div id="group_advert_content">
							<div class="form-group">
								...
								<textarea name="advert_content" id="advert_content" class="validate form-control">{$tplData.advertRow.advert_content}</textarea>
							</div>
						</div>
						...
						<div class="form-group">
							<div id="group_advert_begin">
								...
								<input type="text" name="advert_begin" id="advert_begin" value="{$tplData.advertRow.advert_begin|date_format:"{$smarty.const.BG_SITE_DATE} {$smarty.const.BG_SITE_TIMESHORT}"}" class="validate form-control input_date">
							</div>
						</div>
						...
						<div class="form-group">
							<div id="group_advert_note">
								...
								<input type="text" name="advert_note" id="advert_note" value="{$tplData.advertRow.advert_note}" class="validate form-control">
							</div>
						</div>
			...
			<div class="col-md-3">
				<div class="well">
						...
						{foreach $status.advert as $key=>$value}
							<div class="radio_baigo">
								<label for="advert_status_{$key}">
									<input type="radio" name="advert_status" id="advert_status_{$key}" value="{$key}" class="validate" {if $tplData.advertRow.advert_status == $key}checked{/if} group="advert_status">
									{$value}
								</label>
							</div>
						{/foreach}
					...
					<div class="form-group">
						...
						<select name="advert_put_type" id="advert_put_type" class="validate form-control">
							<option value="">{$lang.option.please}</option>
					...
					<div id="opt_date">
						<div class="form-group">
							<div id="group_advert_put_date">
								...
								<input type="text" name="advert_put_opt" id="advert_put_date" value="{$tplData.advertRow.advert_put_opt|date_format:"{$smarty.const.BG_SITE_DATE} {$smarty.const.BG_SITE_TIMESHORT}"}" class="validate form-control input_date">
							</div>
						</div>
					</div>
					...
					<div id="opt_show">
						<div class="form-group">
							<div id="group_advert_put_show">
								...
								<input type="text" name="advert_put_opt" id="advert_put_show" value="{$tplData.advertRow.advert_put_opt}" class="validate form-control">
							</div>
						</div>
					</div>
					...
					<div id="opt_hit">
						<div class="form-group">
							<div id="group_advert_put_hit">
								...
								<input type="text" name="advert_put_opt" id="advert_put_hit" value="{$tplData.advertRow.advert_put_opt}" class="validate form-control">
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<select name="advert_percent" id="advert_percent" class="validate form-control">
							<option value="">{$lang.option.please}</option>
...
{include "{$smarty.const.BG_PATH_TPL}admin/default/include/admin_foot.tpl" cfg=$cfg}
	...
	var opts_validator_form = {
		advert_name: {
			...
			msg: { id: "msg_advert_name", too_short: "{$alert.x080201}", too_long: "{$alert.x080202}" }
		},
		advert_url: {
			length: { min: 1, max: 3000 },
			validate: { type: "str", format: "text", group: "group_advert_url" },
			msg: { id: "msg_advert_url", too_short: "{$alert.x080205}", too_long: "{$alert.x080206}" }
		},
		advert_posi_id: {
			length: { min: 1, max: 0 },
			validate: { type: "select" },
			msg: { id: "msg_advert_posi_id", too_few: "{$alert.x080203}" }
		},
		advert_media_id: {
			length: { min: 1, max: 0 },
			validate: { type: "digit", format: "int" },
			msg: { id: "msg_advert_media_id", too_small: "{$alert.x080222}", format_err: "{$alert.x080224}" }
		},
		advert_content: {
			length: { min: 1, max: 0 },
			validate: { type: "str", format: "text", group: "group_advert_content" },
			msg: { id: "msg_advert_content", too_short: "{$alert.x080226}" }
		},
		advert_put_type: {
			length: { min: 1, max: 0 },
			validate: { type: "select" },
			msg: { id: "msg_advert_put_type", too_few: "{$alert.x080204}" }
		},
		advert_put_date: {
			length: { min: 1, max: 0 },
			validate: { type: "str", format: "datetime", group: "group_advert_put_date" },
			msg: { id: "msg_advert_put_date", too_short: "{$alert.x080216}", format_err: "{$alert.x080217}" }
		},
		advert_put_show: {
			length: { min: 1, max: 0 },
			validate: { type: "str", format: "int", group: "group_advert_put_show" },
			msg: { id: "msg_advert_put_show", too_short: "{$alert.x080218}", format_err: "{$alert.x080219}" }
		},
		advert_put_hit: {
			length: { min: 1, max: 0 },
			validate: { type: "str", format: "int", group: "group_advert_put_hit" },
			msg: { id: "msg_advert_put_hit", too_short: "{$alert.x080220}", format_err: "{$alert.x080221}" }
		},
		advert_note: {
			length: { min: 0, max: 300 },
			validate: { type: "str", format: "text", group: "group_advert_note" },
			msg: { id: "msg_advert_note", too_long: "{$alert.x080207}" }
		},
		advert_status: {
			length: { min: 1, max: 0 },
			validate: { type: "radio" },
			msg: { id: "msg_advert_status", too_few: "{$alert.x080208}" }
		},
		advert_percent: {
			length: { min: 1, max: 0 },
			validate: { type: "select" },
			msg: { id: "msg_advert_percent", too_few: "{$alert.x080209}" }
		},
		advert_begin: {
			length: { min: 1, max: 0 },
			validate: { type: "str", format: "datetime" },
			msg: { id: "msg_advert_begin", too_short: "{$alert.x080214}", format_err: "{$alert.x080215}" }
		}
	};
