  ...
  $(document).ready(function(){
    ...
    obj_wu.on('uploadSuccess', function(file, result){
        ...
        if (result.rcode == 'y070101') {
          queuedProcess(file.id, 'alert-success', '<?php include($cfg_global['pathIcon'] . 'check-circle' . BG_EXT_SVG); ?>', result.msg);
          ...
          queuedProcess(file.id, 'alert-danger', '<?php include($cfg_global['pathIcon'] . 'times-circle' . BG_EXT_SVG); ?>', result.msg);
        }
    });

    obj_wu.on('uploadError', function(file, result){
      queuedProcess(file.id, 'alert-danger', 'fas <?php include($cfg_global['pathIcon'] . 'times-circle' . BG_EXT_SVG); ?>', 'Error&nbsp;status:&nbsp;' + result);
    });
