    ...
    <dl>
        ...
        foreach ($config['console']['console_mod'] as $_key_m=>$_value_m) { ?>
            ...
            <dd>
                ...
                foreach ($_value_m['allow'] as $_key_s=>$_value_s) {
                    if ($_is_edit) { ?>
                        <div class="form-check form-check-inline">
                            <input type="checkbox" name="admin_allow[<?php echo $_key_m; ?>][<?php echo $_key_s; ?>]" value="1" id="allow_<?php echo $_key_m; ?>_<?php echo $_key_s; ?>" <?php if (isset($_arr_adminRow['admin_allow'][$_key_m][$_key_s])) { ?>checked<?php } ?> data-parent="allow_<?php echo $_key_m; ?>" data-validate="admin_allow" class="form-check-input">
                    ...
                    <?php } else { ?>
                        <span>
                            <span class="fas fa-<?php if (isset($_arr_adminRow['admin_allow'][$_key_m][$_key_s])) { ?>check-circle text-success<?php } else { ?>times-circle text-danger<?php } ?>"></span>
                            <?php echo $lang->get($_value_s, 'console.common'); ?>
                        </span>
                    <?php }
                } ?>
        ...
        <dd>
            ...
            foreach ($config['console']['opt'] as $_key_s=>$_value_s) {
                if ($_is_edit) { ?>
                    <div class="form-check form-check-inline">
                        <input type="checkbox" name="admin_allow[opt][<?php echo $_key_s; ?>]" value="1" id="allow_opt_<?php echo $_key_s; ?>" data-parent="allow_opt" <?php if (isset($_arr_adminRow['admin_allow']['opt'][$_key_s])) { ?>checked<?php } ?> data-validate="admin_allow" class="form-check-input">
                ...
                <?php } else { ?>
                    <span>
                        <span class="fas fa-<?php if (isset($_arr_adminRow['admin_allow']['opt'][$_key_s])) { ?>check-circle text-success<?php } else { ?>times-circle text-danger<?php } ?>"></span>
                        <?php echo $lang->get($_value_s['title'], 'console.common'); ?>
                    </span>
                <?php }
            } ?>
