...
class CONTROL_CONSOLE_UI_LOGIN {
    ...
    function ctrl_sync() {
        ...
        if (defined('BG_SSO_SYNC') && BG_SSO_SYNC == 'on' && isset($this->adminLogged['admin_prefer']['sync']['sync']) && $this->adminLogged['admin_prefer']['sync']['sync'] == 'on') {
    ...
    function ctrl_login() {
        $_str_forward   = fn_getSafe(fn_get('forward'), 'txt', '');
        $_str_jump      = BG_URL_CONSOLE . 'index.php?m=login&a=sync&forward=' . $_str_forward;
