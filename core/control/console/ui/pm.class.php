...
class CONTROL_CONSOLE_UI_PM {
    ...
    function ctrl_show() {
        ...
        $_arr_pmRow['pm_title']     = fn_htmlcode($_arr_pmRow['pm_title'], 'decode', 'json');
        $_arr_pmRow['pm_content']   = fn_htmlcode($_arr_pmRow['pm_content'], 'decode', 'url');

        $_arr_tpl = array(
            'pmRow'   => $_arr_pmRow, //管理员信息
        );
    ...
    function ctrl_send() {
        ...
        if ($_num_pmId > 0) {
            ...
            $_arr_pmRow['pm_title'] = $this->obj_tpl->lang['mod']['label']['reply'] . ': ' . $_arr_pmRow['pm_title'];
            $_arr_pmRow['pm_content'] = PHP_EOL . PHP_EOL . PHP_EOL . '------------------------------------------------' . PHP_EOL . $_arr_pmRow['pm_content'];
        } else {
            $_arr_pmRow = array(
                'pm_id'         => 0,
                'pm_to_user'    => '',
