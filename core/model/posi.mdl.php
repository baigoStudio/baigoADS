...
class MODEL_POSI {
    ...
    public $arr_type        = array('attach', 'text');
    ...
    function mdl_create_table() {
        ...
        $_arr_posiCreat = array(
            ...
            'posi_is_percent'    => 'enum(\'' . $_str_isPercent . '\') NOT NULL COMMENT \'允许按几率展现\'',
    ...
    function mdl_alter_table() {
        ...
        if (in_array('posi_type', $_arr_col)) {
            $_arr_alter['posi_type'] = array('CHANGE', 'enum(\'' . $_str_type . '\') NOT NULL COMMENT \'广告位类型\'', 'posi_type');
        }
        ...
        if (!fn_isEmpty($_arr_alter)) {
            ...
            if (!fn_isEmpty($_reselt)) {
                ...
                $this->obj_db->update(BG_DB_TABLE . 'posi', $_arr_posiData, 'LENGTH(`posi_type`) < 1 OR `posi_type`=\'media\'');
            }
        }
    ...
    function mdl_cache($num_posiId = 0, $is_reGen = false) {
        ...
        if ($is_reGen || !file_exists(BG_PATH_CACHE . 'sys' . DS . 'posi_list.json')) {
            ...
            foreach ($_arr_posiRows as $_key=>$_value) {
                ...
                $_str_outId = fn_jsonEncode($_value);

                $this->obj_file->file_put(BG_PATH_CACHE . 'sys' . DS . 'posi_' . $_value['posi_id'] . '.json', $_str_outId);
            }

            $_str_outList = fn_jsonEncode($_arr_posiRows);

            $_num_size    = $this->obj_file->file_put(BG_PATH_CACHE . 'sys' . DS . 'posi_list.json', $_str_outList);
        }

        if ($num_posiId > 0) {
            if (file_exists(BG_PATH_CACHE . 'sys' . DS . 'posi_' . $num_posiId . '.json')) {
                $_str_cacheReturn = $this->obj_file->file_read(BG_PATH_CACHE . 'sys' . DS . 'posi_' . $num_posiId . '.json');
            } else {
                $_arr_posiRow  = $this->mdl_read($num_posiId);
                $_str_outId    = fn_jsonEncode($_arr_posiRow);
                $this->obj_file->file_put(BG_PATH_CACHE . 'sys' . DS . 'posi_' . $_arr_posiRow['posi_id'] . '.json', $_str_outId);
            }
        } else {
            $_str_cacheReturn = $this->obj_file->file_put(BG_PATH_CACHE . 'sys' . DS . 'posi_list.json');
        }

        $_arr_cacheReturn = fn_jsonDecode($_str_cacheReturn);
    ...
    function mdl_cache_del($arr_posiDels = false) {
        if (is_array($arr_posiDels)) {
            foreach ($arr_posiDels as $_key=>$_value) {
                $this->obj_file->file_del(BG_PATH_CACHE . 'sys' . DS . 'posi_' . $_value . '.json');
            }
        }
    }
    ...
    function mdl_read($str_posi, $str_readBy = 'posi_id', $num_notId = 0) {
        ...
        $_arr_posiRow['posi_opts']    = fn_jsonDecode($_arr_posiRow['posi_opts'], true);
    ...
    function input_submit() {
        ...
        $this->posiSubmit['posi_opts']    = fn_jsonEncode(fn_post('posi_opts'), true);
