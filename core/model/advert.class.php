...
class MODEL_ADVERT {
	 ...
	 * @return void
	 */
	function mdl_list($num_advertNo, $num_advertExcept = 0, $str_key = "", $str_status = "", $num_posi = 0, $is_enable = false, $arr_notIds = false) {
		$_arr_advertSelect = array(
			"advert_id",
			"advert_name",
			"advert_posi_id",
		...
		if ($str_key) {
			$_str_sqlWhere .= " AND (advert_name LIKE '%" . $str_key . "%' OR advert_note LIKE '%" . $str_key . "%')";
		}
		...
		if ($num_posi > 0) {
			$_str_sqlWhere .= " AND advert_posi_id=" . $num_posi;
		}
	 ...
	 * @return void
	 */
	function mdl_count($str_key = "", $str_status = "", $num_posi = 0) {
		$_str_sqlWhere = "1=1";
	...
	function input_ids() {
		...
		$_arr_advertIds = fn_post("advert_id");
