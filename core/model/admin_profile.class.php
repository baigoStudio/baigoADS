...
class MODEL_ADMIN_PROFILE extends MODEL_ADMIN {
    ...
    function mdl_info($num_adminId) {
        ...
        $_num_db = $this->obj_db->update(BG_DB_TABLE . 'admin', $_arr_adminData, '`admin_id`=' . $num_adminId); //更新数据
        if ($_num_db > 0) {
            $_str_rcode = 'y020103'; //更新成功
        } else {
    ...
    function mdl_prefer($num_adminId) {
        $_arr_adminData = array(
            'admin_prefer' => $this->arr_prefer['admin_prefer'],
        );

        $_num_db = $this->obj_db->update(BG_DB_TABLE . 'admin', $_arr_adminData, '`admin_id`=' . $num_adminId); //更新数据
        if ($_num_db > 0) {
            $_str_rcode = 'y020112'; //更新成功
        } else {
            $_str_rcode = 'x020103'; //更新失败
        }
    ...
    function input_prefer() {
        $this->arr_prefer['admin_prefer'] = fn_jsonEncode(fn_post('admin_prefer'), 'no');
