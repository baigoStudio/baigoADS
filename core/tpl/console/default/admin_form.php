                            ...
                            <?php foreach ($this->profile as $key=>$value) { ?>
                                <div class="form-check">
                                    <label for="admin_allow_profile_<?php echo $key; ?>" class="form-check-label">
                                        <input type="checkbox" name="admin_allow_profile[<?php echo $key; ?>]" id="admin_allow_profile_<?php echo $key; ?>" value="1" <?php if (isset($this->tplData['adminRow']['admin_allow_profile'][$key]) && $this->tplData['adminRow']['admin_allow_profile'][$key] == 1) { ?>checked<?php } ?> class="form-check-input">
...
<?php include($cfg['pathInclude'] . 'console_foot.php'); ?>
    ...
    var opts_validator_form = {
        admin_name: {
            ...
            ajax: { url: "<?php echo BG_URL_CONSOLE; ?>index.php?m=admin&c=request&a=chkname", key: "admin_name", type: "str", attach_selectors: ["#admin_id"], attach_keys: ["admin_id"] }
        },
        admin_pass: {
            len: { min: 1, max: 0 },
