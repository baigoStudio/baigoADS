                  ...
                  foreach ($csvRows[0] as $key=>$value) {
                    if (!empty($value)) { ?>
                      <tr>
                        ...
                        <td class="p-2">
                          <span class="bg-icon"><?php include($cfg_global['pathIcon'] . 'arrow-right' . BG_EXT_SVG); ?></span>
                        </td>
                        <td class="p-2">
        ...
        <form name="form_upload" id="form_upload">
          <div class="form-group">
            <button type="button" id="upload_select" class="btn btn-success text-white fileinput-button">
              <span class="bg-icon"><?php include($cfg_global['pathIcon'] . 'cloud-upload-alt' . BG_EXT_SVG); ?></span>
        ...
        <li class="list-group-item">
          ...
          <div class="form-group">
            <button type="button" id="md5_do" class="btn btn-primary">
              <span class="bg-icon"><?php include($cfg_global['pathIcon'] . 'lock' . BG_EXT_SVG); ?></span>
              ...
              <div class="input-group">
                <span class="input-group-prepend">
                  <a href="#charset_list_modal" class="btn btn-warning" data-toggle="modal">
                    <span class="bg-icon"><?php include($cfg_global['pathIcon'] . 'question-circle' . BG_EXT_SVG); ?></span>
                  </a>
                </span>
...
<?php include($cfg['pathInclude'] . 'console_foot' . GK_EXT_TPL); ?>
  ...
  $(document).ready(function(){
    ...
    $('#preview_all').click(function(){
      $.ajax({
        url: '<?php echo $route_console; ?>import/preview/charset/<?php echo $charset; ?>/' + new Date().getTime() + '/' + Math.random() + '/',
        dataType: 'html',
        beforeSend: function(){
          previewProcess();
        },
    ...
    obj_wu.on('fileQueued', function(file){
      $('#csv_upload').html('<div class="alert alert-info bg-alert">' +
        '<h4>' + file.name + '</h4>' +
        '<p class="bg-content">' +
          '<span class="bg-icon bg-fw"><?php include($cfg_global['pathIcon'] . 'cloud-upload-alt' . BG_EXT_SVG); ?></span>' +
    ...
    obj_wu.on('uploadSuccess', function(file, result){
      ...
      if (result.rcode == 'y010403') {
        queuedProcess('alert-success', '<?php include($cfg_global['pathIcon'] . 'check-circle' . BG_EXT_SVG); ?>', '<?php echo $lang->get('CSV file uploaded successfully'); ?>');
      } else {
        ...
        queuedProcess('alert-danger', '<?php include($cfg_global['pathIcon'] . 'times-circle' . BG_EXT_SVG); ?>', _str_msg);
      }
    });

    obj_wu.on('uploadError', function(file, result){
      queuedProcess('alert-danger', '<?php include($cfg_global['pathIcon'] . 'times-circle' . BG_EXT_SVG); ?>', 'Error&nbsp;status:&nbsp;' + result);
    });
